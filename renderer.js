...
let deviceActivityData = []; // rows from device_activity_logs (steps)
let deviceVitalsData   = []; // rows from device_vitals_logs (HR, skin temp, + other unused fields)
...
async function loadData() {
  const [weightsRes, sleepsRes, workoutsRes, customExRes, psychRes, socialRes,
         deviceActivityRes, deviceVitalsRes] = await Promise.all([
...
// Read-only dashboard for device_activity_logs / device_vitals_logs (HR, skin
// temp), which are written by a separate Python sync tool over Bluetooth —
// never by this app. Each chart shows a small "no device data synced yet"
...
// This app only charts heart rate & skin temp from this table (the rest of
// its columns -- SpO2, blood pressure, HRV, EDA -- are pushed by the sync
// tool but not displayed here). Null values are left as null (not coerced to
// 0) so Chart.js draws a gap for that day instead of a misleading zero.
// ─────────────────────────────────────────────────────────────────────────────

// Shared x-axis mapper used by the HR and skin temp charts below.
